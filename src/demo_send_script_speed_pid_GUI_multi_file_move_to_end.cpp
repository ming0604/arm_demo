...
{   
  ...
  ros::init(argc, argv, "demo_move_to_end");      
  ...
  // Create PTP command string to move to the end point
  std::string cmd_end_PTP = "PTP(\"JPP\",";
  std::string PTP_para = PTP_speed + ",100,0,false)";

  for (int i = 0; i < 6; i++){
    end_joint[i] = data.back()[i];
    cmd_end_PTP += std::to_string(data.back()[i]) + ",";
  }

  cmd_end_PTP += PTP_para;
  //cmd_end_PTP += "80,100,0,false)";
  std::cout << "data size: " << data.size() << ", " << data[0].size() << std::endl;
  std::cout << "cmd_end_PTP: "<< cmd_end_PTP << std::endl;
  ...
  // PTP move to the end of the path
  srv.request.id = "demo";
  srv.request.script = cmd_end_PTP;
  if (client.call(srv))                             
  {
    if (srv.response.ok) ROS_INFO_STREAM("end joint => Sent script to robot");
  ...
  ROS_INFO_STREAM("Move to end position command sent. Waiting for robot to reach the position...");
  ...
  while(ros::ok()) 
  {
      ...
      for(int i=0; i<6; i++)
      {
        double err = std::abs(end_joint[i] - joint[i]);
        if(err > max_error) 
        {
          max_error = err;
        }
  ...
  if(reached) 
  {
    ...
    ROS_INFO("Robot has reached the end position within the tolerance.");
